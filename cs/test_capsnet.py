...
class Config:
    def __init__(self, dataset='mnist'):
        if dataset == 'mnist':
            ...
            # Primary Capsule (pc)
            self.pc_num_capsules = 13
            self.pc_in_channels = 256
            self.pc_out_channels = 28
            self.pc_kernel_size = 9
            self.pc_num_routes = 19*28*28

            # Digit Capsule (dc)
            self.dc_num_capsules = 28*13
            self.dc_num_routes = 13*28*28
            self.dc_in_channels = 13
            self.dc_out_channels = 28
            ...
            self.pc_out_channels = 28
            self.pc_kernel_size = 9
            self.pc_num_routes = 28 * 8 * 8

            # Digit Capsule (dc)
            self.dc_num_capsules = 10
            self.dc_num_routes = 28 * 8 * 8
...
if __name__ == '__main__':
    train_dataset = np.load('/home/sathya/Desktop/cs_dataset/vid.npz')['arr_0'][:,-2:,:]
    train_dataset = train_dataset[:,0,:]*train_dataset[:,1,:]
    train_dataset = np.expand_dims(train_dataset,axis=-1)
    cs_train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=1)
